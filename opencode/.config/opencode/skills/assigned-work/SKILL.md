...
1. Show all Jira tickets assigned to me that are not Done. Use `statusCategory != Done AND status != "PO test done"` in the JQL to exclude tickets with "PO test done" status (which is treated as Done even though it belongs to a Done-like category). Display them in a table with columns: Key, Summary, Status, Priority. Sort by priority (Highest first).
